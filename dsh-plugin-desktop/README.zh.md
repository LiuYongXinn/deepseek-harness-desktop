...
未持久化过模式的宿主在 macOS 与 Windows 上默认使用 `advanced`，在 Linux 上默认使用 `compatibility`（高级模式在那里不受支持），因此全新安装会自动进入原生材质呈现，而显式配置的值始终优先。兼容模式创建带有操作系统原生边框的普通窗口，并加载当前 DSH profile 中的官方 Web surface。macOS 会隐藏可见的页面标题。Windows 保留原生标题栏图标并显示 `DeepSeek Harness Desktop`，但会移除窗口菜单栏。原生标题栏颜色与外观由操作系统拥有。
...
完整的分步流程、常见问题与代码更新后的重打包 SOP 见 [docs/packaging-windows.zh.md](docs/packaging-windows.zh.md)。
...
`dist:win` 会拒绝非 Windows 或非 x64 宿主，先执行一组 Windows 可运行的 gate，其中包括 build、全部 TypeScript compiler face、打包与原生 shell 聚焦测试，以及 runtime-closure verifier；随后再构建 NSIS 安装向导，并校验生成的两个 PE 文件。完整跨平台 suite 仍由 CI 持有，因为其中部分 POSIX 执行测试不是 Windows 程序。安装向导支持当前用户安装或提升权限后的所有用户安装，可更改安装目录，会创建开始菜单与桌面快捷方式，并且卸载应用时保留 DSH 用户数据。包版本会输出到 `dsh-plugin-desktop\dist\DSH-Desktop-<version>-x64-Setup.exe`；用于 smoke 测试的未封装程序仍位于 `dsh-plugin-desktop\dist\win-unpacked\DSH Desktop.exe`。
...
- 该项目目前固定使用已发布的 DSH `0.1.0-rc.8` family，与 `upstream.json` 中记录的 `deepseek-harness/` 源码 checkout 一致。因此，测试验证的是已发布包接口，而非上游未发布源码。
