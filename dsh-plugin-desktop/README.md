...
In advanced mode, clicking a supported regular file in the conversation opens a read-only viewer in the right column instead of the operating-system default application. The panel header shows the file name and a truncated full path, and the toolbar provides refresh, system-open, and close icon actions with tooltips and `aria-label`s. Clicking the same path again re-reads it so a change made by the agent can be seen immediately. Directories, unauthorized paths (outside the active workspace membership), unsupported formats, and non-regular files keep the original system behavior with no transient loading surface. Compatibility mode registers no interception, RPC, route, wrapper, style, or viewer UI.

The viewer covers source and config text, Markdown, JSON, diff and patch files, and developer-resource images. Format classification, MIME, and language resolve through the desktop-owned format table. Rendering is isolated per Provider: the Source Provider uses the DSH Shiki/read-block surface with syntax highlighting up to 512 KiB or 10,000 lines (degrading to plain readable text beyond that); the Markdown and JSON Providers preview up to 256 KiB and fall back to source beyond; the Image Provider shows PNG, JPEG, GIF, WebP, and SVG up to 20 MiB, and displays metadata with a system-open action for oversized images. Markdown and JSON offer preview/source and tree/source segmented controls respectively.

The security boundary derives all authorization from workspace membership and reads the registry fresh on each request; a child-agent session inherits only through the authoritative `sessionQuery` ancestry, never a loose session. `fs.resolve`/`contains` verify canonical identity and containment, blocking `..`, absolute, and symlink-escape paths. Control operations use the loopback-only `/desktop-file-preview` RPC channel; image bytes use a same-origin tokenized binary route with the Connection Host/Fetch-Metadata/Origin trust fence. No raw `file://` is exposed to the renderer, no third-party preview service is contacted, and the explicit system-open path is preserved for every preview.

The feature is configured by the advanced-mode-only `filePreview` limits: `maxTextBytes` (default 2 MiB), `maxImageBytes` (default 20 MiB), `resourceTtlMs` (default 60 s), and `maxResources` (default 64). The file viewer adds no new public package exports and no new runtime dependency; jsdom, Testing Library, and React DOM are test-only devDependencies.
...
See [docs/packaging-windows.md](docs/packaging-windows.md) for the full step-by-step flow, common pitfalls, and the repackaging SOP to follow after code changes.
...
- The built-in file viewer is read-only by design. Files beyond a Provider's thresholds (for example oversized images) show metadata and a system-open action rather than a rendered preview; there is no in-place editing or full-featured IDE.
...
- This project currently pins the published DSH `0.1.0-rc.8` family, matching the `deepseek-harness/` source checkout recorded in `upstream.json`. Tests therefore validate the published package interfaces rather than unpublished upstream sources.
