...
- 提供行号、语法高亮、文本选择、复制和浏览器查找。
- 根据扩展名、特殊文件名和 shebang 选择语言。
- 默认文本读取上限为 2 MiB，由 Host Config 显式配置。
- Shiki 高亮上限为 512 KiB 或 10,000 行；超过后降级到普通只读文本，避免主线程长时间阻塞。
- 降级后的普通文本自适应换行（`white-space: pre-wrap`），随面板宽度重排、无换行开关；Desktop 文件查看器通过作用域类使 ReadBlock 高亮路径软换行并禁用横向滚动。Source Provider 通过 Adapter 隔离 Shiki 调用，使 Controller 和文件协议不依赖具体高亮 API。
...
- 源码、文本和 JSON 源码内容自适应换行，随面板宽度重排；Desktop 文件查看器中的 ReadBlock 高亮路径通过作用域类软换行并禁用横向滚动；缩放等数值行为使用明确的图标或步进控件。
...
| `src/client/file-preview/providers/SourcePreview.tsx` | 将 descriptor 与文本转换为 `ReadBlock` 输入；实现高亮门限与普通文本降级，两条路径均自适应换行，高亮路径通过 Desktop 作用域类禁用横向滚动。 |
