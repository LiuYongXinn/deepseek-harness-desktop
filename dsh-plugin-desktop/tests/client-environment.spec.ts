...
describe('advanced desktop layout', () => {
  it('owns native caption geometry without targeting feature headers', () => {
    ...
    try {
      ...
      expect(css).toMatch(/\.dshDesktopSourceBody\s*\{[^}]*overflow-x:\s*hidden;[^}]*overflow-y:\s*auto;/)
      expect(css).toMatch(/\.dshDesktopSourcePlain\s*\{[^}]*overflow-x:\s*hidden;[^}]*overflow-y:\s*auto;/)
      expect(css).toMatch(/\.dshDesktopSourceReadBlock\s*\{[^}]*min-width:\s*0;[^}]*max-width:\s*100%;/)
      expect(css).toMatch(/\.dshDesktopSourceReadBlock > div:last-child > div\s*\{[^}]*align-items:\s*flex-start;[^}]*white-space:\s*pre-wrap;/)
      expect(css).toMatch(/\.dshDesktopSourceReadBlock > div:last-child > div > span:last-child\s*\{[^}]*min-width:\s*0;[^}]*overflow-wrap:\s*anywhere;/)
      expect(css).not.toContain('[data-read]')
